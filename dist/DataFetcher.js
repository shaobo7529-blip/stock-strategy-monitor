...
async function withRetry(fn, symbol, retryCount = DEFAULT_RETRY_COUNT, retryIntervalMs = DEFAULT_RETRY_INTERVAL_MS) {
    let lastError = null;
...
export function calculateDailyChanges(prices, symbol) {
    if (prices.length < 2)
        return [];
    const changes = [];
