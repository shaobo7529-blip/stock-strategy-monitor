 ...
 */
export function calculateVolumeRatio(prices) {
 ...
 */
export function calculateMAAlignment(prices) {
    // 需要至少 50 条数据来计算 MA50
    if (prices.length < 50) {
        return null;
    }
...
export function calculateKDJ(prices) {
    ...
    for (let i = period - 1; i < prices.length; i++) {
        ...
        for (let j = i - period + 1; j <= i; j++) {
            if (prices[j].high > high9)
                high9 = prices[j].high;
            if (prices[j].low < low9)
                low9 = prices[j].low;
        }
 ...
 */
export function calculateBollinger(prices) {
    const period = 20;
    const multiplier = 2;
    // 需要至少 20 条数据
 ...
 * - normal_volume → 0
 */
function scoreVolumeRatio(result) {
    switch (result.signal) {
        case 'high_volume':
            return 8;
        case 'low_volume':
 ...
 * - tangled → 0
 */
function scoreMaAlignment(result) {
    switch (result.signal) {
        case 'bullish':
            return 17;
        case 'bearish':
 ...
 */
function scoreBollinger(result) {
    switch (result.signal) {
        case 'below_lower':
            return 17;
        case 'above_upper':
