...
export function deserialize(csv) {
...
export class TriggerTracker {
    records = [];
    constructor(existingCsv) {
        if (existingCsv !== undefined && existingCsv.trim() !== '') {
            try {
                this.records = deserialize(existingCsv);
            }
            catch {
                this.records = [];
            }
    ...
    updatePerformance(symbol, triggerDate, nextDayChange, maxGainIn5Days, day5Change) {
    ...
    /** 兼容旧接口 */
    updateNextDayPerformance(symbol, triggerDate, performance) {
        this.updatePerformance(symbol, triggerDate, performance, performance, performance);
    }
    getPendingTriggers() { return this.records.filter((r) => r.status === 'pending'); }
    getAllRecords() { return [...this.records]; }
    toCSV() { return serialize(this.records); }
}
//# sourceMappingURL=TriggerTracker.js.map