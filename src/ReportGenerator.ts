...
export function generateCSV(records: TriggerRecord[]): string {
...
export function calculateStats(records: TriggerRecord[], strategyName: string): StrategyStats {
  const completed = records.filter(
    (r) => r.status === 'completed' && r.strategyType === strategyName && r.nextDayChange !== null
  );
  ...
  const nextDayChanges = completed.map((r) => r.nextDayChange as number);
  ...
  const maxGains5 = with5Day.map((r) => r.maxGainIn5Days as number);
...
export function generateConsoleSummary(records: TriggerRecord[]): string {
  if (records.length === 0) return '无触发记录';
  const strategyTypes = [...new Set(records.map((r) => r.strategyType))].sort();
  const lines: string[] = ['=== 策略监控摘要 ===', ''];
