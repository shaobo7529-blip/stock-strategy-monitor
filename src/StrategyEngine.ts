 ...
 */
export class SingleDayDropStrategy implements Strategy {
 ...
 */
export class UnderperformBenchmarkStrategy implements Strategy {
 ...
 */
export class RSI2OversoldStrategy implements Strategy {
  ...
  evaluate(stock: DailyChange, _benchmark: DailyChange | null, threshold: number, context?: DailyChange[]): boolean {
    ...
    if (recentChanges.length < period) return false;
 ...
 * 需要历史上下文数据
 */
export class ConsecutiveDownDaysStrategy implements Strategy {
  ...
  evaluate(stock: DailyChange, _benchmark: DailyChange | null, threshold: number, context?: DailyChange[]): boolean {
    ...
    if (stock.changePercent < 0) {
      ...
      for (let i = context.length - 1; i >= 0; i--) {
        if (context[i].changePercent < 0) count++;
        else break;
      }
    }
 ...
 */
export class CumulativeRSI2Strategy implements Strategy {
  ...
  evaluate(stock: DailyChange, _benchmark: DailyChange | null, threshold: number, context?: DailyChange[]): boolean {
    ...
    if (rsiToday === null || rsiYesterday === null) return false;

    // 累积 RSI = 最近 2 天的 RSI(2) 之和
    const cumulativeRSI = rsiToday + rsiYesterday;
    if (cumulativeRSI >= threshold) return false;
  ...
  private calcRSI2(prev: DailyChange, curr: DailyChange): number | null {
 ...
 * 大盘恐慌性下跌后，个股反弹概率高
 */
export class VIXSpikeStrategy implements Strategy {
...
export class StrategyEngine {
   ...
   */
  evaluate(
    stockChanges: DailyChange[],
    benchmarkChanges: DailyChange[],
    ...
    for (let idx = 0; idx < stockChanges.length; idx++) {
      ...
      for (const cfg of enabledConfigs) {
        ...
        if (cfg.type === 'underperform-benchmark' && benchmark === null) continue;
