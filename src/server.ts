...
import type { Configuration, DailyChange, DailyPrice, TriggerRecord, TechnicalAnalysisResponse } from './types.js';
import {
  calculateMACD,
  calculateRSI,
  ...
  for (const symbol of validSymbols) {
    ...
    for (const event of events) {
      ...
      if (event.strategyType !== 'ma-pullback' && event.strategyType !== 'vix-spike' && event.strategyType !== 'hammer-reversal') {
        ...
        if (drop >= -1 || drop < -3) continue; // 只保留跌幅在1-3%的信号
      }
...
// --- 扫描状态 ---
let scanResult: { date: string; signals: any[]; scanTime: string } | null = null;
...
async function runDailyScan() {
  if (isScanning) { console.log('Scan already running, skip'); return; }
  isScanning = true;
  ...
  if (spyResult.ok && spyResult.value.length >= 200) {
    ...
    const ma200 = last200.reduce((s: number, p: any) => s + p.close, 0) / 200;
  ...
  for (let i = 0; i < LARGE_CAP_SYMBOLS.length; i++) {
    ...
    try {
      ...
      const todayEvents = events.filter((e: any) => e.triggerDate === lastDate);
      
      for (const event of todayEvents) {
        // Apply SPY bull filter (skip non-vix strategies in bear market)
        if (!spyBull && event.strategyType !== 'vix-spike') continue;
        
        // Apply IBS filter
        const priceIdx = result.value.findIndex((p: any) => p.date === event.triggerDate);
        if (priceIdx >= 0 && event.strategyType !== 'ma-pullback' && event.strategyType !== 'vix-spike' && event.strategyType !== 'hammer-reversal') {
...
const server = http.createServer(async (req, res) => {
        ...
        if (config.groups) {
          for (const g of config.groups) {
            for (const sym of (g.symbols || [])) {
              if (!config.stockList.includes(sym)) config.stockList.push(sym);
            }
          }
        }
    ...
    try {
      ...
      try {
        ...
        for (const item of emItems) {
          const jys = item.JYS;
          let suffix = '';
          if (jys === '71' || item.SecurityTypeName?.includes('沪')) suffix = '.SS';
          else if (jys === '80' || item.SecurityTypeName?.includes('深')) suffix = '.SZ';
          else if (jys === '116' || item.SecurityTypeName?.includes('港')) suffix = '.HK';
          if (suffix) {
  ...
  if (pathname === '/api/scan' && req.method === 'GET') {
    ...
    } catch { sendJSON(res, 200, { scan: null, scanning: isScanning }); }
    return;
  }

  // API: Trigger manual scan
  if (pathname === '/api/scan' && req.method === 'POST') {
    if (isScanning) { sendJSON(res, 200, { ok: false, message: 'Scan already running' }); return; }
  ...
  if (pathname?.startsWith('/api/technical/')) {
    ...
    try {
      ...
      const response: TechnicalAnalysisResponse = {
        symbol,
        date: lastPrice.date,
